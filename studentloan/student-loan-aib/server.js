...
app.post('/calculate-loan', (req, res) => {
    ...
    const monthlyPayment = (loanAmount * interestRate) / loanTermLength;
...
app.listen(port, async () => {
    ...
    console.log(`AIB Student loan service listening at http://student-loan-aib:${port}`);

    // Register with the control server
    try {
        const res = await axios.post('http://student-loan-registry:6000/register', {
            serviceName: 'student-loan-aib',
            // serviceUrl: `http://studentloan-aib:${port}`
            serviceUrl: `http://student-loan-aib:${port}`
        });
